...
from cryptography.fernet import Fernet  # Correct import statement
import hashlib
import os
import pickle
...
# Sidebar menu
menu = st.sidebar.radio("Select an option", ["Home", "Store Data", "Retrieve Data", "Login"])
...
# [Rest of your code remains exactly the same...]        except Exception as e:
...
if choice == "Home":
    ...
    with st.form("login_form"):
        ...
        if st.form_submit_button("Login"):
            if hashlib.sha256(login_pass.encode()).hexdigest():
                st.session_state.failed_attempts = 0
                save_data({
                    "stored_data": st.session_state.stored_data,
                    "failed_attempts": st.session_state.failed_attempts
